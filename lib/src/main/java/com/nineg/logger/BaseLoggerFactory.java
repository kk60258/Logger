...
public class BaseLoggerFactory {
    private static final ConcurrentMap<String, BaseLoggerBase> LOGGER_MAP = new ConcurrentHashMap<String, BaseLoggerBase>();
    private static final int TAG_MAX_LENGTH = 23; // tag names cannot be longer on Android platform

    public static BaseLoggerBase getLogger(final String name)
    {
        final String tag = getValidName(name); // fix for bug #173

        BaseLoggerBase logger = LOGGER_MAP.get(tag);
        if (logger != null) return logger;

        logger = new BaseLoggerBase(tag);
        BaseLoggerBase loggerPutBefore = LOGGER_MAP.putIfAbsent(tag, logger);
